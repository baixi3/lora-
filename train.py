...
def collate_fn(batch):
    ...
    # 将 input_ids 复制为 labels，并先屏蔽 padding 位置，避免 pad token 参与 loss
    model_inputs["labels"] = model_inputs["input_ids"].clone()
    model_inputs["labels"][model_inputs["attention_mask"] == 0] = -100

    # 只监督 assistant 后的 ans 部分，这里做了一个切片
    ...
    for idx, input_ids in enumerate(model_inputs["input_ids"]):
        ...
        if len(assistant_positions) > 0:
            ...
            model_inputs["labels"][idx, :start_pos] = -100 # huggingface 定义的 -100 token为不监督
        else:
            # 处理没有找到的情况（例如屏蔽全部）
            model_inputs["labels"][idx, :] = -100
    # print(model_inputs["pixel_values"].shape)
    # print(f"model_inputs['pixel_values'].shape: {model_inputs['pixel_values'].shape}")
    return model_inputs
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        epoch_iterator = tqdm(dataloader, desc=f"Epoch {epoch+1}", leave=True)
    ...
    # 保存 LoRA 权重到一个字典中 idx 为 name, value 为 param
    lora_weights = {
